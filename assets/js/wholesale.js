/* ESCENA wholesale (mayorista) — currently DISABLED.
   The mayorista program isn't live yet, so there are NO accounts and NO
   credentials of any kind shipped in this file. login() always fails, which
   keeps the 15%-off mode permanently off. The pricing helpers below stay in
   place (dormant) so the storefront keeps working unchanged; re-enable the
   program later by wiring these to a real server-side auth check — never by
   putting passwords back in the page source. */
(function () {
  ...
  // No accounts while the mayorista program is disabled.
  var ACCOUNTS = [];
  ...
  /* login(email, password) -> {ok, role}. Mayorista program disabled: no
     accounts exist, so this always reports invalid credentials. */
  function login() {
    return { ok: false };
  }
